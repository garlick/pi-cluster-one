...
// begin with global mask (0 to allow everything)
EXTERNC void can1_begin (uint32_t idmask);
EXTERNC void can1_end (void);

// must set all filters if using any, n = 0...7
EXTERNC void can1_setfilter (uint32_t idmask, uint8_t n);

EXTERNC int can1_available (void);

EXTERNC int can1_write (uint32_t id, uint8_t len, uint8_t *buf,
                        uint16_t timeout);

EXTERNC int can1_read (uint32_t *id, uint8_t *len, uint8_t *buf,
                       uint16_t timeout);
